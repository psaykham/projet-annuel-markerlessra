...
- (void)processFrame
{
    ...
    //NSLog(@"rows=%i / cols=%i",m_oldFrame.rows, m_oldFrame.cols );
    //cv::Mat test = cvCreateMat(m_oldFrame.rows, m_oldFrame.cols, m_oldFrame.type());
    ...
    std::vector<cv::KeyPoint> oldKeypointList, newKeypointList;
    detector.detect(*m_oldFrame, oldKeypointList);
    detector.detect(*m_newFrame, newKeypointList);*/
    ...
    cv::Mat oldDescriptor;
    extractor.compute(m_oldFrame, oldKeypointList, oldDescriptor);
    
    cv::Mat newDescriptor;
    extractor.compute(m_newFrame, newKeypointList, newDescriptor);*/
