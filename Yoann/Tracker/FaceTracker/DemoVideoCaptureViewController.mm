...
- (void)processFrame:(cv::Mat &)mat videoRect:(CGRect)rect videoOrientation:(AVCaptureVideoOrientation)videOrientation
{
    ...
    mat.copyTo(m_lastFrame);
    mat.resize(mat.rows/4, mat.cols/4);
    rect.size.width /= 4;
    rect.size.height /= 4;
    [self computeFrame:mat];
}

-(void)computeFrame:(cv::Mat &)newFrame
{
    if(!m_lastFrame.empty() && !newFrame.empty())
    {
        // Detecing Keypoints with FAST algorithme
        std::vector<cv::KeyPoint> oldKeypointList = [self DetectKeypoint: &m_lastFrame];
        std::vector<cv::KeyPoint> newKeypointList = [self DetectKeypoint: &newFrame];
    
        // Computing Descriptors
        //cv::Mat oldDescriptor = [self ComputeDesciptor:&m_lastFrame :&oldKeypointList];
        //cv::Mat newDescriptor = [self ComputeDesciptor:&newFrame :&newKeypointList];
    }
}
